...
import waterSaving from "@/app/data/waterSave";
import productCards from "@/app/data/productCards";
import productQuestion from "@/app/data/productsQuestion";
    ...
    <>
      <section>
        {filterRecord.length > 0 ? (
          filterRecord.map((item) => {
            return (
              <div key={item.id}>
                <div className="home-container sm:p-[1rem] md:p-[4rem] sm:mb-[1rem] md:mb-[5rem]">
                  <h1 className="text-center text-white sm:text-[20px] md:text-[42px] font-medium sm:leading-[25px] md:leading-[60px] sm:mb-[1rem] md:mb-[3rem]">
                    {item.mainHeading}
                  </h1>
                  <p className="text-center text-white sm:text-[15px] sm:leading-[20px] md:text-[24px] font-medium md:leading-[40px]">
                    {item.mainSubHeading}
                  </p>
                </div>
                ...
                <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:px-[1rem] md:px-[4rem] sm:mb-[3rem] md:mb-[5rem]">
                  <div className="flex items-center">
                    <Image
                      src={saveWaterIcon}
                      className="sm:w-[50px] md:w-auto"
                    />{" "}
                    <span className="ms-4">{item.productBenefitOne}</span>
                  </div>
                  <div className="flex items-center">
                    <Image
                      src={saveWaterIcon2}
                      className="sm:w-[35px] md:w-auto"
                    />{" "}
                    <span className="ms-4">{item.productBenefitTwo}</span>
                  </div>
                  <div className="flex items-center">
                    <Image
                      src={weatherIcon}
                      className="sm:w-[50px] md:w-auto"
                    />{" "}
                    <span className="ms-4">{item.productBenefitThree}</span>
                  </div>
                  <div className="flex items-center">
                    <Image
                      src={electricPolIcon}
                      className="sm:w-[50px] md:w-auto"
                    />{" "}
                    <span className="ms-4">{item.productBenefitFour}</span>
                  </div>
                  <div className="flex items-center">
                    <Image
                      src={equalizerIcon}
                      className="sm:w-[50px] md:w-auto"
                    />{" "}
                    <span className="ms-4">{item.productBenefitFive}</span>
                  </div>
                  <div className="flex items-center">
                    <Image
                      src={roiWeekIcon}
                      className="sm:w-[50px] md:w-auto"
                    />{" "}
                    <span className="ms-4">{item.productBenefitSix}</span>
                  </div>
                ...
                <div className="grid grid-cols-1 text-center sm:mb-[3rem] md:mb-[5rem]">
                  <p className="sm:hidden md:flex justify-center items-center mb-[3rem]">
                    <span className="text-PriceText text-[24px] font-medium leading-[40px] me-[2rem]">
                      {item.unitText}
                    </span>{" "}
                    <span className="text-[64px] font-medium leading-[40px]">
                      {item.UnitAmount}
                    </span>
                  </p>
                  <div className="sm:block md:hidden">
                    <p className="text-[30px] font-medium leading-[20px] mb-[1.5rem]">
                      {item.UnitAmount} PKR
                    </p>
                    <p className="text-PriceText text-[16px] font-medium leading-[20px] mb-[1.5rem]">
                      {item.unitText}
                    </p>
                  </div>
                  <button className="sm:px-[8px] sm:py-[5px] md:px-[20px] md:py-[12px] sm:text-[20px] md:text-[36px] font-medium leading-[40px] bg-BuyBtn hover:bg-buyBtnHover text-white sm:w-[150px] md:w-[300px] mx-auto rounded-[46px]">
                    Buy Now
                  </button>
                </div>
              </div>
      ...
      <section>
        <h1 className="text-center mb-[2rem] font-bold text-subHeadingColor sm:text-[20px] md:text-[25px]">
          Relevant Products
        </h1>
        <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4 sm:px-4 md:px-[4rem] sm:mb-[2rem] md:mb-[5rem]">
          {productCard.length > 0 ? (
            productCard.slice(0, 4).map((item) => {
              return (
                <Link href={`/product/${item.id}`}>
                  <div className="text-center shadow-card-shadow sm:px-[6px] sm:py-[10px] md:p-[15px] rounded-[10px]">
                    ...
                    <p className="sm:text-[12px] md:text-[16px] font-medium sm:min-h-[120px] md:min-h-[135px] sm:leading-[16px] md:leading-[25px]">
                      {item.cardSubHeading}
                    </p>
                    <h1 className="sm:text-[15px] md:text-[20px] font-bold mb-[1rem]">
                      {item.cardPrice} PKR
                    </h1>
                    <button
                      className="bg-BuyBtn hover:bg-buyBtnHover text-white font-bold sm:text-[12px] md:text-[16px] sm:px-[12px] sm:py-[5px] md:px-[20px] md:py-[12px] rounded-[45px]"
                      onClick={() => hiddenProductCardhandler(item.id)}
                    >
                      {item.cardBtnLabel}
                    </button>
                  </div>
